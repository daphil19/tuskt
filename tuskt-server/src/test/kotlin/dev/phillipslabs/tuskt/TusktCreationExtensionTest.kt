...
class TusktCreationExtensionTest {
    ...
    @Test
    fun testCreationWithUploadLength() = testApplication {
    ...
    @Test
    fun testCreationWithZeroLength() = testApplication {
        application { creationModule() }
        val response = client.post("/files") {
            header(TusHeaders.TUS_RESUMABLE, SUPPORTED_TUS_VERSIONS.first())
    ...
    @Test
    fun testCreationMissingLengthHeaders() = testApplication {
        application { creationModule() }
        val response = client.post("/files") {
            header(TusHeaders.TUS_RESUMABLE, SUPPORTED_TUS_VERSIONS.first())
        }
    ...
    @Test
    fun testCreationMissingTusResumable() = testApplication {
        application { creationModule() }
        val response = client.post("/files") {
            header(TusHeaders.UPLOAD_LENGTH, "100")
        }
    ...
    @Test
    fun testCreationWithDeferLength() = testApplication {
    ...
    @Test
    fun testCreationWithInvalidDeferLength() = testApplication {
        application { creationModule(deferLength = true) }
        val response = client.post("/files") {
            header(TusHeaders.TUS_RESUMABLE, SUPPORTED_TUS_VERSIONS.first())
    ...
    fun testHeadDeferredResourceEchoesHeader() = testApplication {
        application { creationModule(deferLength = true) }
        val createResponse = client.post("/files") {
            header(TusHeaders.TUS_RESUMABLE, SUPPORTED_TUS_VERSIONS.first())
            header(TusHeaders.UPLOAD_DEFER_LENGTH, "1")
        }
        val id = extractId(createResponse)

        val headResponse = client.head("/files/$id") {
            header(TusHeaders.TUS_RESUMABLE, SUPPORTED_TUS_VERSIONS.first())
        }
    ...
    @Test
    fun testCreationWithMetadata() = testApplication {
        application { creationModule() }
    ...
    @Test
    fun testCreationWithUpload() = testApplication {
    ...
    @Test
    fun testCreationWithUploadPartial() = testApplication {
        application { creationModule(enableUpload = true) }
        val response = client.post("/files") {
            header(TusHeaders.TUS_RESUMABLE, SUPPORTED_TUS_VERSIONS.first())
            header(TusHeaders.UPLOAD_LENGTH, "100")
    ...
    @Test
    fun testCreationWithUploadDisabled() = testApplication {
    ...
    fun testCreationWithUploadWrongContentType() = testApplication {
        application { creationModule(enableUpload = true) }
        val response = client.post("/files") {
            header(TusHeaders.TUS_RESUMABLE, SUPPORTED_TUS_VERSIONS.first())
            header(TusHeaders.UPLOAD_LENGTH, "10")
    ...
    @Test
    fun testOptionsAdvertisesCreation() = testApplication {
    ...
    @Test
    fun testOptionsAdvertisesDeferLength() = testApplication {
    ...
    @Test
    fun testOptionsAdvertisesCreationWithUpload() = testApplication {
